...
class VahysPerceptron:
    ...
    def __init__(self, vectorSize, learningRate=0.01):
        ...
        initBiases = np.asarray(np.random.normal(0, 1, 1), dtype=theano.config.floatX)
    ...
    def fit(self, data, labels, numOfIterations=1):
        ...
        # labels = theano.shared(np.asarray(labels, dtype=theano.config.floatX))

        X = T.dmatrix(name='X')
        y = T.dvector(name='y')
        potential = T.dot(X, self.W) + self.biasVisible
        # activation = 1 / (1 + T.exp(potential))
        activation = potential
        cost = T.sum((activation - y) ** 2)
        gw, gb = T.grad(cost, [self.W, self.biasVisible])
        train = theano.function(inputs=[X, y], outputs=[activation, cost], updates=[[self.W, self.W - 0.01 * gw], [self.biasVisible, self.biasVisible - 0.01 * gb]], name="asdf")
